...
function VitePluginRemoteAssets(options = {}) {
    ...
    for (const rule of rules) {
      ...
      while (match = rule.match.exec(code)) {
        ...
        if (!url || !isValidHttpUrl(url))
          continue;
        const hash = (0, import_blueimp_md5.default)(url) + (rule.ext || (0, import_path.extname)(url));
        const filepath = slash((0, import_path.resolve)(dir, hash));
        ...
        let newUrl;
        if (mode === "relative") {
          newUrl = slash((0, import_path.relative)((0, import_path.dirname)(id), `${dir}/${hash}`));
          if (newUrl[0] !== ".")
            newUrl = `./${newUrl}`;
        } else {
          let path = `${dir}/${hash}`;
  ...
  return {
    ...
    async configResolved(_config) {
      config = _config;
      dir = slash((0, import_path.resolve)(config.root, assetsDir));
